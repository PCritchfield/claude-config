...
```
claude/.claude/
├── CLAUDE.md           # Compressed Rincewind personality + universal rules (~44 lines)
├── settings.json       # Claude Code settings: model, permissions, env vars, hooks, spinners
├── rules/              # Path-scoped rule files (loaded contextually, not always)
│   ├── council-charter.md     # Promotion path, output gates, conflict resolution
│   ├── council-agents.md      # Agent roster, mandatory triggers, model allocation
│   ├── sdd-workflow.md        # SDD stages, TDD enforcement
│   ├── obsidian.md            # Session summaries, vault search via MCP
│   ├── ci-preflight.md        # CI validation before pushing
│   ├── agent-preflight.md     # Parallel agent dispatch checklist
│   └── post-implementation.md # Polish workflow, merge readiness
├── agents/             # Watch Council agent definitions (12 agents)
├── hooks/              # Lifecycle hooks
│   ├── session-start-context.sh  # Loads prior Obsidian session as context
│   └── precompact-checkpoint.sh  # Saves checkpoint before context compaction
├── commands/           # SDD commands (generated by slash-man, gitignored)
└── skills/             # Custom skills (community skills installed separately, gitignored)
    ├── obsidian-digest/
    ├── obsidian-summary/
    ├── review/
    └── polish/
...
Setup has three steps. Each is independent — run what you need.
...
Creates symlinks from `~/.claude/` pointing into the repo using [GNU Stow](https://www.gnu.org/software/stow/). Requires `stow` (`brew install stow`). The script automatically backs up any pre-existing real files before replacing them with symlinks.
...
Installs 27 community skills from the `skills.txt` manifest via `npx skills add`. These are domain skills used by the Watch Council agents (security, architecture, database, IaC, etc.).

**Prerequisite:** Install the [obra/superpowers](https://github.com/nicholasgriffintn/superpowers) plugin first — it provides 14 core skills (systematic-debugging, test-driven-development, etc.) that overlap with the manifest. Install via Claude Code: `/plugins` → search "superpowers" → install.
...
Generates the `/SDD-1` through `/SDD-4` slash commands from the [spec-driven-workflow](https://github.com/liatrio-labs/spec-driven-workflow) repository.

### Step 4: Obsidian MCP Server (optional)

If you use Obsidian for session notes:

1. Install the [Local REST API plugin](https://github.com/coddingtonbear/obsidian-local-rest-api) in Obsidian (Community Plugins → Browse → "Local REST API")
2. Enable it and copy the API key from its settings
3. Add the MCP server:
   ```bash
   claude mcp add obsidian --scope user -- npx -y obsidian-mcp-server
   ```
4. Set the environment variables in `~/.claude.json`:
   ```json
   "obsidian": {
     "env": {
...
The main personality file (`CLAUDE.md`, ~44 lines) contains the compressed Rincewind persona and universal rules (Plan Mode default, output attribution, context discipline). Everything else lives in `.claude/rules/` as separate files:

- **council-charter.md** — Promotion path (PLAN → PROBE → IMPLEMENT), output gates, conflict resolution, veto hierarchy
- **council-agents.md** — Agent roster, mandatory consult triggers, model allocation, institutional memory guidance
- **sdd-workflow.md** — SDD stages, promotion within SDD-3, TDD enforcement
- **obsidian.md** — Session summary rules, vault search patterns via MCP
- **ci-preflight.md** — CI validation before pushing (ruff, prettier, pyright)
- **agent-preflight.md** — Parallel agent dispatch checklist
- **post-implementation.md** — Polish workflow sequence

Rules files load contextually — they're always available but separated by concern for maintainability. The previous monolithic CLAUDE.md (462 lines) was split to stay within the ~150-200 instruction compliance ceiling for frontier LLMs.

## Hooks

Three lifecycle hooks in `settings.json`:

| Hook | Event | Purpose |
|------|-------|---------|
| `session-start-context.sh` | SessionStart | Loads the most recent Obsidian session note for the current project/branch as `additionalContext`. Ends cold starts. |
| `precompact-checkpoint.sh` | PreCompact | Saves a lightweight checkpoint marker to the Obsidian vault before context compression. Non-blocking, best-effort. |
| PostToolUse (inline) | Write/Edit | Auto-formats with ruff (Python) and prettier (JS/TS/CSS/JSON) on changed files. |
...
Four hand-maintained skills. Community skills (33 from various repos) are installed separately via `install_skills.sh` and gitignored — they create symlinks into `~/.agents/skills/` and are not tracked in this repo.
...
Post-implementation workflow that orchestrates: create PR → run `/simplify` and `/review` in parallel → push fixes → wait for external reviewers → surface tech debt as GitHub issues. Suggested automatically after SDD-3 tasks complete.
...
- `hooks` — SessionStart, PreCompact, and PostToolUse hooks (see Hooks section)
- `autoMemoryEnabled` / `autoDreamEnabled` — automatic memory and consolidation
...
The Obsidian MCP Server configuration (including the API key) lives in `~/.claude.json` at user scope, not in this file — it is machine-specific and not tracked in git.
...
SDD stage commands (`/SDD-1-generate-spec` through `/SDD-4-validate-spec-implementation`) are generated by [slash-man](https://github.com/liatrio-labs/slash-command-manager) from the [spec-driven-workflow](https://github.com/liatrio-labs/spec-driven-workflow) repository. They are gitignored and generated during setup (see Step 3 above).
